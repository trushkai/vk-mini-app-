...
import yellowBasketImg from '../../assets/baskets/wide-yellow-basket.svg';
import pinkBasketImg   from '../../assets/baskets/wide-pink-basket.svg';

/* ---------- типы ---------- */
type ItemType = 'green' | 'yellow' | 'pink';
...
const initialItems: DraggableItem[] = [
  ...
  { id:'3', type:'pink',   img: pinkFlower   },
  { id:'4', type:'yellow', img: money        },
  { id:'5', type:'green',  img: secretBottle },
  { id:'6', type:'pink',   img: pinkCrystal  },
];

/* ======================================================================= */
const Level8: FC<GameWrapperProps> = ({
  currentLevel, openModal, onLevelComplete, activeModal,
}) => {
  /* состояние предметов */
  const [items,       setItems]       = useState(initialItems);
  const [greenDone,   setGreenDone]   = useState<DraggableItem[]>([]);
  const [yellowDone,  setYellowDone]  = useState<DraggableItem[]>([]);
  const [pinkDone,    setPinkDone]    = useState<DraggableItem[]>([]);
  ...
  const onDragEnd = (e:DragEndEvent)=>{
    ...
    if(over.id === basketId){
      ...
      switch(dragged.type){
        ...
        case 'yellow': setYellowDone(p=>[...p,dragged]); break;
        case 'pink':   setPinkDone  (p=>[...p,dragged]); break;
      }
      ...
      <div className="game-container">
        ...
        <DndContext onDragStart={onDragStart} onDragEnd={onDragEnd}>
          ...
          <div className="baskets-row">
            ...
            <div className="wide-yellow-basket"><Basket type="yellow" items={yellowDone}/></div>
            <div className="wide-pink-basket"><Basket type="pink"   items={pinkDone}/></div>
          </div>
